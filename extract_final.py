...
def process_excel_sheets_to_csv(base_folder_path, row_indices, column_indices_map, output_csv):
    # Adding 'Folder Name' to the headers
    headers = ['Sheet Name', 'Folder Name', 'Capital Fund 23', 'Capital Fund 22', 'Income 23', 'Income 22', 'Expenses 23', 'Expenses 22']
    all_data = []
    ...
    for root, dirs, files in os.walk(base_folder_path):
        for file in files:
            if file.endswith(".xlsx"):
                ...
                # Extract folder name from the root
                folder_name = os.path.basename(root)
                try:
                    xls = pd.ExcelFile(file_path)
                    for sheet_name in xls.sheet_names:
                        sheet_data = [sheet_name]  # Start with the sheet name
                        # Insert the folder name into the sheet data
                        sheet_data.append(folder_name)
                        try:
